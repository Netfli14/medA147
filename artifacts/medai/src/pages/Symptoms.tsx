  ...
  const analyzeSymptoms = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/ai/analyze-symptoms", {
        ...
        body: JSON.stringify({ symptoms: allSymptoms, profileContext: fullContext, language }),
      });
              ...
              {results.healthScore !== undefined && results.riskScore !== undefined && (
                <HealthDashboard
                  data={{
                    ...
                    topCondition: results.conditions?.[0]?.name,
                    verdictEvidence: results.verdictEvidence,
                  }}
                />
              )}
