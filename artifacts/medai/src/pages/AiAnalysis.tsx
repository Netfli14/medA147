...
/** Extract [label](url) links from text into an array */
  ...
  const analyzeImage = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/ai/analyze-image", {
        ...
        body: JSON.stringify({ image, profileContext, language }),
      });
          ...
          {results && !isAnalyzing && (
            <div className="mt-12 space-y-8 animate-fade-up">
                      ...
                      <div key={index} className="flex items-start gap-3 p-3 rounded-xl bg-muted/50">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm text-muted-foreground leading-relaxed">
                            {translatedObservations[index] ?? observation}
                          </p>
                          <TranslateButton
                            text={observation}
                            onTranslated={(v) => setTranslatedObservations((prev) => ({ ...prev, [index]: v }))}
                            isTranslated={!!translatedObservations[index]}
                            cacheKey={`obs:${index}:${observation.slice(0, 60)}`}
                            className="mt-1 h-6 text-[11px] opacity-60 hover:opacity-100"
                          />
                        </div>
                      </div>
                    ))}
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    {results.conditions.map((condition, index) => {
                      ...
                      const displayDesc = translatedConditions[index] ?? condition.description;
                      return (
                          ...
                          <div className="flex items-start justify-between gap-4 mb-3">
                            ...
                            <div className="flex items-center gap-1 shrink-0">
                              <TranslateButton
                                ...
                                onTranslated={(v) => setTranslatedConditions((prev) => ({ ...prev, [index]: v }))}
                                isTranslated={!!translatedConditions[index]}
                                cacheKey={`cond:${index}:${condition.name}`}
                                className="h-8 text-xs"
                              />
              ...
              {results.recommendation && (() => {
                const recLinks = extractLinks(results.recommendation);
                return (
                  <div className="glass-card p-6 rounded-2xl border-2 border-primary/20 bg-gradient-to-br from-primary/5 to-transparent">
                    <div className="flex items-start gap-4">
                      <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-primary/10">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-2 flex-wrap mb-2">
                          <h3 className="font-display text-lg font-bold text-foreground">{t('aiRecommendation')}</h3>
                          <TranslateButton
                            text={results.recommendation}
                            onTranslated={setTranslatedRecommendation}
                            isTranslated={!!translatedRecommendation}
                            cacheKey={`rec:${results.recommendation.slice(0, 60)}`}
                            className="h-8 text-xs"
                          />
                        </div>
                        <p className="text-muted-foreground leading-relaxed text-sm">
                          {translatedRecommendation
                            ? translatedRecommendation
                            : renderWithLinks(results.recommendation)}
                        </p>
                        {recLinks.length > 0 && !translatedRecommendation && (
                          <div className="mt-3">
                            <p className="text-[11px] uppercase tracking-wide font-semibold text-muted-foreground flex items-center gap-1 mb-1.5">
                              <BookOpen className="h-3 w-3" />
