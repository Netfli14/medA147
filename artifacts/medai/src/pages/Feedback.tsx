...
export default function Feedback() {
  ...
  const fetchSuggestions = useCallback(async () => {
    ...
    try {
      ...
      const mapped: Suggestion[] = (data || []).map((s: any) => ({
        ...
        suggestion_en: s.suggestionEn,
        suggestion_ru: s.suggestionRu,
        suggestion_kk: s.suggestionKk,
        suggestion_zh: s.suggestionZh,
        original_lang: s.originalLang,
        created_at: s.createdAt,
        likes_count: s.likes ?? 0,
        liked_by_me: (s.replies ?? []).some
          ? false
          : false,
      ...
      // Check local storage for likes this visitor has made
      const myLikesRaw = localStorage.getItem('medai-my-likes');
      const myLikes: string[] = myLikesRaw ? JSON.parse(myLikesRaw) : [];
      mapped.forEach(s => { s.liked_by_me = myLikes.includes(s.id); });
  ...
  const handleLike = async (suggestionId: string, currentlyLiked: boolean) => {
    ...
    // Persist like state in local storage
    const myLikesRaw = localStorage.getItem('medai-my-likes');
    const myLikes: string[] = myLikesRaw ? JSON.parse(myLikesRaw) : [];
    if (currentlyLiked) {
      localStorage.setItem('medai-my-likes', JSON.stringify(myLikes.filter(id => id !== suggestionId)));
    } else {
      localStorage.setItem('medai-my-likes', JSON.stringify([...myLikes, suggestionId]));
    }
    try {
      await fetch(`/api/feedback/suggestions/${suggestionId}/like`, {
        method: 'POST',
  ...
  const handleReply = async (suggestionId: string) => {
    ...
    try {
      ...
      const res = await fetch(`/api/feedback/suggestions/${suggestionId}/reply`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          replyText: text,
          authorName: isCreator ? 'MedAI+ Team' : name,
          isCreator,
        }),
      });
      if (!res.ok) throw new Error(await res.text());
      const { data: newReplyData } = await res.json();

      const newReply: Reply = {
        id: newReplyData?.id ?? crypto.randomUUID(),
        reply_text: text,
        author_name: isCreator ? 'MedAI+ Team' : name,
        is_creator: isCreator,
        created_at: newReplyData?.createdAt ?? new Date().toISOString(),
      };
